...
func CheckClusterRoles(clusterConfig model.Drifter, client kubernetes.Interface, ctx context.Context) bool {
	hasDrifts := false
	var driftCount = 0
	if len(clusterConfig.Kubernetes.ClusterRoles) > 0 {

		for _, ds := range clusterConfig.Kubernetes.ClusterRoles {

			dsList, err := client.RbacV1().ClusterRoles().List(ctx, v1.ListOptions{})
			if err != nil {
				log.Fatal("Unable to get ClusterRoles ", err)
			}
			if len(dsList.Items) == 0 {
				...
				if result.Equal(required) {
					...
					for _, d := range diffs.ToSlice() {
						log.Printf("Missing ClusterRoles %s\n", d)
					}
				}
			}
		}
