...
func CheckDeployments(clusterConfig model.Drifter, client kubernetes.Interface, ctx context.Context) bool {
	hasDrifts := false
	var driftCount = 0
	if len(clusterConfig.Kubernetes.Deployments) > 0 {
		for _, depls := range clusterConfig.Kubernetes.Deployments {

			dsList, err := client.AppsV1().Deployments(depls.NameSpace).List(ctx, v1.ListOptions{})
			if err != nil {
				log.Fatal("Unable to get Deployments ", err)
			}
			if len(dsList.Items) == 0 {
				...
				if result.Equal(required) {
					...
					for _, d := range diffs.ToSlice() {
						log.Printf("Missing Deployment %s\n", d)
					}
				}
